...
// Satisfaction creates a new Satisfaction problem
func Satisfaction(variant string, n int) *discrete.Problem {
	name := newName(SAT, variant, n)
	switch variant {
	case "langford":
		return satLangfordPair(name, n)
	case "magicseries":
		return satMagicSeries(name, n)
	case "nqueens":
		return satNQueens(name, n)
	default:
		return nil
	}
}
...
func satLangfordPair(name string, n int) *discrete.Problem {
...
func satMagicSeries(name string, n int) *discrete.Problem {
	p := discrete.NewProblem(name)
...
// N-Queens problem
func satNQueens(name string, n int) *discrete.Problem {
	p := discrete.NewProblem(name)
	p.Type = discrete.Sequence
	p.Goal = discrete.Satisfy
